...
//findWord - this function is the second stop of the search - gets a term to search in the posting file
function findWord($term){
  ...
  //connection established - now do query
   if (strpos($term, '!') !== false){        //if word contains the NOT
    ...
    $query = "SELECT * FROM postingfile NATURAL JOIN documents WHERE documents.R_id not in 
              (SELECT documents.R_id FROM postingfile NATURAL JOIN documents WHERE word = '$term') 
              GROUP BY file_id ORDER BY hits DESC";   //prepare query  
    } else {
      $query = "SELECT * FROM postingfile  WHERE word_id = '$term' ORDER BY hits DESC";   //prepare query
    }
...
function findStopWord($term){
  ...
  if (strpos($term, '!') !== false){        //if word contains the NOT
    ...
    $query = "SELECT * FROM stop_list NATURAL JOIN documents WHERE word = '$term' ORDER BY hits DESC";   //prepare query
  }
  $result = mysqli_query($connection, $query);    //execute it
  $result = mysqli_fetch_all($result, MYSQLI_ASSOC);        //fetch all answers Assoc - all field names
  if ($result) return $result;      //if exist, return it
